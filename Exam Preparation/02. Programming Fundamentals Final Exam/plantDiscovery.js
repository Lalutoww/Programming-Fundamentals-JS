function plantDiscovery(input) {
   ...
   while (command != 'Exhibition') {
      ...
      if (plantCollection[plantName]) {
         switch (task) {
            case 'Rate':
               plantCollection[plantName].rating.push(Number(value));
               break;
            case 'Update':
               plantCollection[plantName].rarity = Number(value);
               break;
            case 'Reset':
               plantCollection[plantName].rating = [];
         }
      } else {
         console.log('error');
      }
